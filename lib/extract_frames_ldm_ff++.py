...
def preprocess_video(video_path, save_path, face_detector, face_predictor):
    ...
    frame_idxs = np.linspace(FRAME_IDXS_START, frame_count_video - 1, NUM_FRAMES, endpoint=True, dtype=int)
    # process each frame
    ...
    for dataset in DATASETS:
        for comp in COMPRESSION:
            ...
            for i in tqdm(range(n_sample)):
                save_path_per_video = (movies_path_list[i].replace(VIDEO_PATH, SAVE_IMGS_PATH).replace('.mp4', '').replace("/videos", "/frames"))

                preprocess_video(movies_path_list[i], save_path_per_video,face_detector, face_predictor)
