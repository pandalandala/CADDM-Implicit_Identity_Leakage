...
class DeepfakeDataset(Dataset):
    ...
    Args:
        mode: train or test.
        config: hyperparameters for processing images.
    """
    ...
    def collect_samples(self) -> List:
        ...
        for key in sorted(self.class_dict.keys()):
            d = os.path.join(directory, key).replace(os.path.sep, '/')
            if not os.path.isdir(d):
                continue
            for r, _, filename in sorted(os.walk(d, followlinks=True)):
                for name in sorted(filename):
                    path = os.path.join(r, name).replace(os.path.sep, '/')
                    info_key = path[:-4]
    ...
    def collect_class(self) -> Dict:
        ...
        # classes = [d.name.replace(os.sep, '/') for d in os.scandir(self.root) if d.is_dir()]
        # classes = [Path(d.name) for d in os.scandir(self.root) if d.is_dir()]
        # classes = [cls.as_posix() for cls in classes]
        # for cls in classes:
        #     print("1"+cls)
        classes.sort(reverse=True)
        # for i in range(len(classes)):
        #     print ({classes[i]: np.int32(i)})
